...
<div class="form-group">
  <label class="col-md-4 control-label" for="txtstockmaximo">Stock M&aacute;ximo: </label>  
  <div class="col-md-4">
  <input id="txtstockmaximo" name="txtstockmaximo" type="number" value="<?php echo $value->STOCK_MINIMO;?>"
   placeholder="Stock m&aacute;ximo del productos" class="form-control input-md" required="">
  </div>
</div>
<div class="form-group">
...
<div class="form-group">
  <label class="col-md-4 control-label" for="txttipoprod">Tipo Producto: </label>  
  <div class="col-md-8">
  <?php if(is_array($tipo_producto)  && count($tipo_producto)>0){?>
  <select id="select_tipoproducto" name="select_tipoproducto" class="form-control">
    <option value="">SELECCIONE</option>
  <?php
      foreach($tipo_producto as $value_tip){
      
        echo '<option value="'.$value_tip->ID.'" '.($value_tip->ID ==  $value->TIPO_PRODUCTO_ID ? 'selected' : '').' >'.$value_tip->DESCRIPCION.'</option>';
      }
      ?>
      </select>
